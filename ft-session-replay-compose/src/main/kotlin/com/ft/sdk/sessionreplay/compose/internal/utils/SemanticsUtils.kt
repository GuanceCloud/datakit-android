...
import com.ft.sdk.sessionreplay.RateBasedSampler
import com.ft.sdk.sessionreplay.TextAndInputPrivacy
import com.ft.sdk.sessionreplay.TouchPrivacy
import com.ft.sdk.sessionreplay.compose.ImagePrivacySemanticsPropertyKey
import com.ft.sdk.sessionreplay.compose.SessionReplayHidePropertyKey
import com.ft.sdk.sessionreplay.compose.TextInputSemanticsPropertyKey
import com.ft.sdk.sessionreplay.compose.TouchSemanticsPropertyKey
import com.ft.sdk.sessionreplay.compose.internal.data.BitmapInfo
import com.ft.sdk.sessionreplay.compose.internal.isLeafNode
import com.ft.sdk.sessionreplay.compose.internal.isPositionedAtOrigin
import com.ft.sdk.sessionreplay.compose.internal.mappers.semantics.TextLayoutInfo
import com.ft.sdk.sessionreplay.utils.GlobalBounds

@Suppress("TooManyFunctions")
internal class SemanticsUtils(
    private val reflectionUtils: ReflectionUtils = ReflectionUtils(),
    private val sampler: RateBasedSampler = RateBasedSampler(BITMAP_TELEMETRY_SAMPLE_RATE)
) {
    ...
    private fun resolveOuterBounds(semanticsNode: SemanticsNode): GlobalBounds {
        ...
        semanticsNode.layoutInfo.getModifierInfo().filter {
            ...
            currentBounds = GlobalBounds(
                currentBounds.x - start.toLong(),
                currentBounds.y - top.toLong(),
                currentBounds.width + (end + start).toLong(),
                currentBounds.height + (bottom + top).toLong()
            )
        }
    ...
    internal fun resolveClipCornerRadius(semanticsNode: SemanticsNode): Float? {
        val currentBounds = resolveOuterBounds(semanticsNode)
        if (!isGlobalBoundsValid(currentBounds)) {
            return null
        }
        val density = semanticsNode.layoutInfo.density
        val graphicsLayerModifier = semanticsNode.layoutInfo.getModifierInfo().firstOrNull {
            reflectionUtils.isGraphicsLayerElement(it.modifier)
        }?.modifier ?: return null
        return resolveClipShape(graphicsLayerModifier, currentBounds, density)
    }
    ...
    private fun logUnsupportedPainter(painter: Painter?) = Unit
    ...
    private fun resolveTextStringSimpleElementOverflow(semanticsNode: SemanticsNode): TruncationMode? {
    ...
    private fun resolveTextOverflow(overflowMode: Int): TruncationMode? {
        return when (overflowMode) {
            TEXT_OVERFLOW_CLIP -> TruncationMode.CLIP
            TEXT_OVERFLOW_ELLIPSE -> TruncationMode.TAIL
            TEXT_OVERFLOW_VISIBLE -> null
            TEXT_OVERFLOW_ELLIPSIS_START -> TruncationMode.HEAD
            TEXT_OVERFLOW_ELLIPSIS_MIDDLE -> TruncationMode.MIDDLE
            else -> {
    ...
    private fun logReflectionExtractionFailure(overflowValue: Any, e: Throwable) = Unit

    private fun logUnknownOverflowOrdinal(ordinal: Int) = Unit

    private fun logUnknownOverflowType(overflowValue: Any) = Unit

    private fun convertTextLayoutInfo(
        layoutInput: TextLayoutInput,
        multiParagraphCapturedText: String?,
        modifierColor: Color?,
        textOverflow: TruncationMode?
    ): TextLayoutInfo {
