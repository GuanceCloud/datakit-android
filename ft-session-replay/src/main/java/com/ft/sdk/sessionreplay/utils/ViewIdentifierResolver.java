...
/**
 * Resolves stable identifiers used to correlate Android views with Session Replay wireframes.
 */
public interface ViewIdentifierResolver {
     ...
     * the parent {@link View} tag to provide consistency. In case there was already a value with the
     * same key in the tags and this was used by a different party we will try to use this value
     * as identifier if it is a {@link Long}, in other case we will return null. This last scenario is
     * highly unlikely but we are doing this in order to safely treat possible collisions with
     * client tags.
     * @param parent the parent {@link View} of the virtual child
     * @param childName the unique name of the virtual child
     * @return the unique identifier as {@link Long} or null if the identifier could not be created
     */
