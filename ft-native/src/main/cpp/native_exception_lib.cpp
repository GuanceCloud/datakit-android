...
/// call java uploadCrash code

static void uploadCrash(char *crash) {
    if (!env) {
        return;
    }
    ...
    jmethodID uploadMid = env->GetMethodID(ftExceptionHandler, "uploadNativeCrashLog",
                                           "(Ljava/lang/String;)V");
    env->CallVoidMethod(job, uploadMid, char2JString(env, crash));
}
